...
I've been trying to figure out the format of the data on the memory cards, with a view to writing my own designs to a custom card (TLDR - I've not got that far).  The [pyembroidery library](https://github.com/EmbroidePy/pyembroidery), which is used by InkStitch can read the .sew files which (I thought) my machine used, and write .jef files (which is the format used by newer Janome machines, which has some similarities with .sew files).
...
![](images/designs_02_0x0962.png)

![](images/colours_02_0x36ac.png)

There are also images for the touch-screen's icons:

![](images/controls_02_0x440d.png)

And some text in Japanese:

![](images/japanese_02_0x566f.png)

I assume this is ghost data from a different card.
...
Could possibly modify a PCMCIA card (CF adaptor? But those are a little thicker).  The pins on the machine should be longer, given the ones on the PCMCIA socket were too short.  
